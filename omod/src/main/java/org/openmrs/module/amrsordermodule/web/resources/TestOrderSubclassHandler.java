 ...
 */
@SubClassHandler(supportedClass = TestOrder.class, supportedOpenmrsVersions = { "2.0.*", "2.1.*", "2.2.*", "2.4.*", "2.8.*"})
public class TestOrderSubclassHandler extends BaseDelegatingSubclassHandler<Order, TestOrder> implements DelegatingSubclassHandler<Order, TestOrder> {
	...
	public PageableResult getActiveOrders(Patient patient, RequestContext context) {
		...
		if (StringUtils.isNotBlank(sortParam)) {
			List<Order> sortedOrder = sortOrdersBasedOnDateActivatedOrDateStopped(testOrders, sortParam,
			    status);
