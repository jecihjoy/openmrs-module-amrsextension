...
//		        + " WHERE  ((t1.voided IS NULL || t1.voided = 0)  AND t6.uuid = '974b082f-8d2a-4cb9-9334-18b494c2118a' ) GROUP BY t1.order_id ";
//
		...
		"t12.name AS display, "+
				"t13.uuid AS conceptUuid,  "+
		"t1.uuid AS uuid,  "+
				"t7.uuid AS obs_uuid,  "+
		"t1.date_activated AS date_activated,  "+
				"t1.patient_id AS patient_id,  "+
		"t9.name AS sample_drawn,  "+
				"DATE(t7.obs_datetime) AS sample_collection_date,  "+
		"t3.given_name AS given_name,  "+
				"t3.middle_name AS middle_name,  "+
		"t3.family_name AS family_name,  "+
				"GROUP_CONCAT(DISTINCT t4.identifier) AS identifiers,  "+
		"CONCAT(COALESCE(t3.given_name, ''), ' ',  "+
				"COALESCE(t3.middle_name, ''), ' ',  "+
				"COALESCE(t3.family_name, '')) AS person_name,  "+
		"t5.name AS location_name,  "+
				"t5.uuid AS locationUuid,  "+
		"t5.location_id AS location_id,  "+
				"t6.uuid AS patient_uuid,  "+
		"t10.uuid AS provider_uuid,  "+
				"CONCAT(COALESCE(t11.given_name, ''), ' ',  "+
						"COALESCE(t11.middle_name, ''), ' ',  "+
						"COALESCE(t11.family_name, '')) AS provider_name,  "+
		"GROUP_CONCAT(DISTINCT contacts.value  "+
				"SEPARATOR ', ') AS phone_number  "+
		"FROM  "+
		"orders `t1`  "+
		"LEFT OUTER JOIN  "+
		"encounter `t2` ON (t1.encounter_id = t2.encounter_id)  "+
		"LEFT OUTER JOIN  "+
		"person_name `t3` ON (t1.patient_id = t3.person_id  "+
				"AND (t3.voided IS NULL || t3.voided = 0))  "+
		"LEFT OUTER JOIN  "+
		"patient_identifier `t4` ON (t1.patient_id = t4.patient_id)  "+
		"LEFT OUTER JOIN  "+
		"location `t5` ON (t2.location_id = t5.location_id)  "+
				"LEFT OUTER JOIN  "+
		"person `t6` ON (t1.patient_id = t6.person_id)  "+
		"LEFT OUTER JOIN  "+
		"obs `t7` ON (t1.order_id = t7.order_id  "+
				"AND (t7.voided IS NULL || t7.voided = 0))  "+
		"LEFT OUTER JOIN  "+
		"concept_name `t8` ON (t7.concept_id = t8.concept_id)  "+
		"LEFT OUTER JOIN  "+
		"concept_name `t9` ON (t7.value_coded = t9.concept_id)  "+
		"LEFT OUTER JOIN  "+
		"provider `t10` ON (t1.orderer = t10.provider_id)  "+
		"LEFT OUTER JOIN  "+
		"person_name `t11` ON (t10.person_id = t11.person_id)  "+
		"LEFT OUTER JOIN  "+
		"concept `t13` ON (t1.concept_id = t13.concept_id)  "+
		"LEFT OUTER JOIN  "+
		"concept_name `t12` ON (t1.concept_id = t12.concept_id)  "+
		"LEFT OUTER JOIN  "+
		"person_attribute `contacts` ON (t1.patient_id = contacts.person_id  "+
				"AND (contacts.voided IS NULL  "+
						"|| contacts.voided = 0)  "+
				"AND contacts.person_attribute_type_id IN (10 , 48))  "+
		"LEFT OUTER JOIN  "+
		"person_address `pa` ON (t1.patient_id = pa.person_id)  "+
		"WHERE  "+
				"(t6.uuid = '974b082f-8d2a-4cb9-9334-18b494c2118a'  "+
